...
public class NoteController {
	...
	Note notes = context.getBean("note", Note.class);
	...
	{
		List<Note> note1;
		note1=noteRepository.getAllNotes();
		map.addAttribute("notes",note1);
		return "index";
	}
	...
	*/
	...
	public String indexSave(ModelMap map, @RequestParam int noteId, @RequestParam String noteTitle, @RequestParam String noteContent, @RequestParam String noteStatus)
	{
		notes = context.getBean("note", Note.class);
		notes.setNoteId(noteId);
		notes.setNoteTitle(noteTitle);
		notes.setNoteContent(noteContent);
		notes.setNoteStatus(noteStatus);
		notes.setCreatedAt(LocalDateTime.now());
		List<Note> note2;
		noteRepository.addNote(notes);
		note2=noteRepository.getAllNotes();
		map.addAttribute("notes",note2);
		return "index";
	...
	*/
	...
	public String deleteExistingNote(ModelMap map, @RequestParam int noteId)
	{
